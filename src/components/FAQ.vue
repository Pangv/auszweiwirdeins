<script setup lang="ts">
</script>

<template>
  <section class="py-10 md:py-20 px-6 md:px-12 text-left bg-primary text-secondary">
    <div class="max-w-7xl mx-auto">
      <h2 class="font-extrabold text-[clamp(2.5rem,10vw,6rem)] mb-12 text-left leading-none darker">FAQ</h2>

      <div class="grid md:grid-cols-2 gap-16 items-center">
        <div class="space-y-8">
          <div>
            <h3 class="text-2xl font-display uppercase text-accent mb-2">Dresscode</h3>
            <p class="opacity-80">Hauptsache Bunt! Alles ist erlaubt - je farbenfroher, desto besser.</p>
            <p class="opacity-80 italic">Sie trägt weiß, er was grünes.</p>
          </div>
          <div>
            <h3 class="text-2xl font-display uppercase text-accent mb-2">Kinder</h3>
            <p class="opacity-80 text-xl">Ja, sehr gerne.</p>
          </div>
          <div>
            <h3 class="text-2xl font-display uppercase text-accent mb-2">Geschenke</h3>
            <p class="opacity-80">Wir freuen uns am meisten über eure Anwesenheit. Falls ihr uns dennoch etwas schenken möchtet, freuen wir uns über einen Beitrag zu unseren Flitterwochen.</p>
          </div>
          <div>
            <h3 class="text-2xl font-display uppercase text-accent mb-2">Locations</h3>
            <p class="opacity-80">Die Trauung findet im Standesamt statt, die Feier im Yolk am Kanal.</p>
          </div>
          <div>
            <h3 class="text-2xl font-display uppercase text-accent mb-2">Reden & Partyspiele</h3>
            <p class="opacity-80">Bitte sprecht euch dazu mit unseren Trauzeugen ab.</p>
          </div>
        </div>

        <div class="relative">
          <div class="absolute -inset-4 border-2 border-accent/30 rounded-3xl -rotate-2"></div>
          <img src="https://images.unsplash.com/photo-1542044801-30d3e45ae49a?q=80&w=1740&auto=format&fit=crop"
               class="rounded-3xl shadow-2xl relative z-10 w-full object-cover h-[500px]"
               alt="FAQ Dekoration">
        </div>
      </div>
    </div>
  </section>
</template>
